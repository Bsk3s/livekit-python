...
async def create_spiritual_token(request: SpiritualTokenRequest, _: bool = Depends(verify_api_key)):
    ...
    try:
        ...
        logger.info(
            f"Created spiritual token for {request.user_name} with {request.character} in room {room_name}"
        )
...
async def create_token_legacy(request: dict):
    ...
    try:
        ...
        return {"token": response.token, "room": response.room_name}
...
async def generate_token_expo(request: dict):
    ...
    try:
        ...
        spiritual_request = SpiritualTokenRequest(
            character=character, user_id=identity, user_name=identity, session_duration_minutes=30
        )
        ...
        return {
            ...
            "wsUrl": "wss://hb-j73yzwmu.livekit.cloud",  # Correct LiveKit WebSocket URL with camelCase field name
        }
