...
class CustomTTSService(tts.TTS):
    ...
    async def synthesize(self, text: str) -> "tts.ChunkedStream":
        ...
        try:
            ...
            return tts.ChunkedStream(
                self._convert_to_audio_frames(audio_data)
            )
    ...
    def _convert_to_audio_frames(self, audio_data: np.ndarray) -> AsyncGenerator[rtc.AudioFrame, None]:
...
def create_custom_tts(character: str = "default") -> CustomTTSService:
    ...
    return CustomTTSService(
        model_path="path/to/your/model",  # TODO: Set your model path
