...
async def comprehensive_openai_test():
    """Comprehensive OpenAI TTS latency test"""
    print("🚀 Comprehensive OpenAI TTS Latency Test")
    ...
    for config in configs:
        ...
        for test_name, test_text in test_cases:
            ...
            try:
                ...
                status = "🎯 TARGET MET!" if response_time < 1500 else "❌ Too slow"
    ...
    if results:
        ...
        if valid_results:
            ...
            avg_openai = sum(r["response_time_ms"] for r in valid_results) / len(valid_results)
            print(f"   OpenAI Best: {best_openai:.0f}ms")
            print(f"   OpenAI Average: {avg_openai:.0f}ms")
    
    print(f"\n💡 KEY INSIGHTS:")
    print(f"   ✅ OpenAI TTS quality is excellent")
    print(f"   ❌ OpenAI generates ENTIRE audio first (no true streaming)")
    print(f"   ✅ Deepgram streams audio as it's generated")
    print(f"   🎯 For real-time conversation: Deepgram wins on latency")
    
    if passed >= total_tests * 0.8:
        print("\n🚀 OpenAI TTS meets latency requirements!")
        return True
    else:
        print("\n💀 OpenAI TTS has latency challenges for real-time use")
        return False

if __name__ == "__main__":
    result = asyncio.run(comprehensive_openai_test()) 