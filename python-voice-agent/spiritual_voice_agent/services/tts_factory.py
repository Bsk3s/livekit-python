...
TTS_MODEL = os.getenv("TTS_MODEL", "openai")  # openai, custom, gemini, elevenlabs
    ...
    def create_tts(character: str = "default", model_override: Optional[str] = None) -> tts.TTS:
        ...
        try:
            if model == "openai":
                ...
                logger.warning(f"⚠️ Unknown TTS model: {model}, falling back to OpenAI")
                return TTSFactory._create_openai_tts(character)
...
def test_tts_model(model: str, character: str = "adina", test_text: str = "Hello, this is a test.") -> bool:
    """
